...
def proto_net_episode_for_inference(model: Module,
                      optimiser: Optimizer,
                      loss_fn: Callable,
                      x: Tuple[torch.Tensor, torch.Tensor, torch.Tensor],  # Tuple chứa support, labels, query
                      y: torch.Tensor,
                      n_shot: int,
                      k_way: int,
                      q_queries: int,
    ...
    # Split tuple for only tensor
    support_tensors, _, query_tensors = x

    # Embedding all sample
    ...
    # Calculate distance
    distances = pairwise_distances(query_embeddings, prototypes, distance)
